...
public func />> <T, U, V, E> (f: T -> Result<U, E>, g: U -> Result<V, E>) -> T -> Result<V, E> {
    return {
        f($0).flatMap { x in g(x) }
    }
}

public func />> <T, U, V, E> (f: T -> Result<U, E>, g: Result<U, E> -> V) -> T -> V {
    return {
        g(f($0))
    }
}

public func />> <T, U, V, E> (f: T -> Result<U, E>, g: U -> V) -> T -> Result<V, E> {
    return {
...
public func |>> <T, U, E> (f: T -> Result<U, E>, g: Result<U, E> -> Void) -> T -> Void {
    return {
        f($0) |> g
        return
    }
